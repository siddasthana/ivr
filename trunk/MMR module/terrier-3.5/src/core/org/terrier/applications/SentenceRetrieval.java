 ...
 * under the License.
 *
 * The Original Code is SentenceRetrieval.java.
 *
 ...
 */
public class SentenceRetrieval {
    
    int fpointer;    
    HashMap<Integer,RSHolder> hp=new HashMap();
    ResultSet parentrs,sim1set,set,receivedset,finalset;
    Sim1 obj=new Sim1();
     String name[];
     String document;  
    int theta=50;
     ...
     int docid1;
     static String[] args1;
    private int numberReqd;// this is the number u can set to limit no. of final results
    private int gettheta(ResultSet rs) {
        
       return 50;
    }
        CollectionResultSet qrs=new CollectionResultSet(100);
      //  ResultSet rs= qrs.getResultSet(0, 100);
        /*int[] docid=qrs.getDocids();
        double[] score=qrs.getScores();
        short[] occurence=qrs.getOccurrences();
        int resultSizes=qrs.getResultSize();
        int exactResultSizes=qrs.getExactResultSize();
        */
        /*
         *querys- contains the current query
        */
     
	/** The logger used */
	protected static final Logger logger = Logger.getLogger(SentenceRetrieval.class);
	...
	*/
	...
	public void printResults(PrintWriter pw, SearchRequest q) throws IOException {
		 parentrs=q.getResultSet();
                 theta=gettheta(parentrs);
                 fpointer=0;
                 int count=0;
                 if(parentrs.getResultSize()>numberReqd)
                     count=parentrs.getResultSize();
                 else
                     count=numberReqd;
                 ...
                 sim1set = parentrs.getResultSet(0,theta);
                 docid=sim1set.getDocids();
            //     System.out.println(" first docid is : "+docid[0]);
                 docid1=docid[0];
                 set=MMR(sim1set);
