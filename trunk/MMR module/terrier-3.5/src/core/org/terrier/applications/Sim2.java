 ...
 * under the License.
 *
 * The Original Code is Sim2.java.
 *
 ...
 */
public class Sim2 {
    ResultSet rs,sim1set,set;
    static ResultSet globalSet;
	...
	/** The logger used */
	protected static final Logger logger = Logger.getLogger(Sim2.class);
	...
	*/
	...
	public void printResults(PrintWriter pw, SearchRequest q) throws IOException {
		 rs=q.getResultSet();
                theta=gettheta(rs);
                sim1set = rs.getResultSet(0,theta);
                set=MMR(sim1set);
                System.out.println("  printing set :"+set);
                globalSet=set;
                 System.out.println("  printing set :"+globalSet);
                docid=sim1set.getDocids();
                System.out.println("printing docid where global set initialised"+docid);
                docid1=docid[0];
