 ...
 */
public class SentenceRetreival1 {
	/** The logger used */
	protected static final Logger logger = Logger.getLogger(SentenceRetreival1.class);
	...
	*/
	...
	public static void main(String[] args) {
		SentenceRetreival1 iq = new SentenceRetreival1();
		if (args.length == 0)
		{
        ...
        String docpath,doccontent,s;
        int k;
        int[] documentids;
public void collectorFn() throws FileNotFoundException,IOException 
{
    documentids=set.getDocids();
    for(int i=0;i<k;i++)
    {
        docpath=set.getMetaItem("filename",documentids[i]);
        FileReader fr=new FileReader(docpath);
        BufferedReader read=new BufferedReader(fr);
        while((s=read.readLine())!=null)
        {
            doccontent=doccontent.concat(s);
        }
        /*
         * send this doc (doccontent) as a query now.
         * and get that resultSet
         * retreive the 2nd doc from this retreived resultset and
         * compare its docid with the original resultset docid.
         * if it matches than place this doc at 2nd position in the final result set
         * with 1st doc thier being the 1st doc recieved from the query.
         * 
         * Now again with the 2nd doc added in the final resultset,take that as
         * query and send it to the original index.And continue this procedure till k.
        */
    }
