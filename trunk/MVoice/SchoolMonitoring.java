...
public class SchoolMonitoring implements FreeswitchScript, DTMFCallback,
		HangupHook {
	...
	public void start(JavaSession ss, String cid) {
		...
		Date st = new Date();
		String Query = "Update Call_history set Ring_Duration ='"
				+ ((st.getTime() - dt.getTime()) / 1000) + "' where id=" + cid;
		update(Query);
		Query = "Update Call_history set Hangup_Cause='"
				+ session.hangupCause() + "' where id=" + cid;
		update(Query);
		Query = "Update Call_history set Call_Duration='"
		...
		// if(Checkforregistration(Caller)){ //Enabled calling to unregistered
		// number
		Query = "Insert into Call_schedule (Caller, Schedule, Timeout, App_id, Status) values('"
				+ Caller
	...
	private boolean IS_Voulenteer(String clr) {
		// TODO Auto-generated method stub
		String Query = " Select * from Subscriber_subscriber where phone ='"
				+ clr.substring(clr.length() - 10) + "'";
		Connection con = new DatabaseHandler().getConnection();
		ResultSet Gateways;
		try {
	...
	public void call() {
		...
		try {
			...
			if (Gateways.next()) {
				gateways = Gateways.getInt(1); // number of gateways retrieved
												// from database;
			}
		...
		try {
			...
			if (rs.next()) {
				...
				while (rst.next()) {
					...
					Debug(Query);
					ResultSet chkvalid = con.createStatement().executeQuery(
							Query);
					if (!chkvalid.next()) {
						continue;
					}
					...
					if (session != null) {
						...
						if (registered) {
							...
							if (voulenteer) {
								while (session.ready()) {
									code = session.read(3, 3, sd + "22.mp3",
											1400, "#", 0);
									if (code.length() == 3) {
										try {
											...
											session.streamFile(sd
													+ "invalid.mp3", 0);
											// TODO: handle exception
										}
									} else {
										session.streamFile(sd + "invalid.mp3",
												0);
									}
								}
							}
							...
							write("Callerid:" + Caller + ", Time:"
									+ new Timestamp(date.getTime())
									+ ", Absent: " + Absent + ", Meal: " + Meal
									+ ", Code:" + code);
							Caller = "";
							Absent = "";
							Meal = "";
