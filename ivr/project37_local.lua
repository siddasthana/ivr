...
query = "Select I_question.QuestionID, file from I_question, I_assignment where I_question.QuestionID = I_assignment.questionid  AND I_assignment.assignedto =" .. userid .. " AND I_assignment.replied = 'n'";
reply_Q(query);
end
-----------------
-- reply Q
...
--database updation
--------------
query1 = "insert into I_answer (questionid ,posting_date , repliedby, file) values (" .. Qid .. ",curdate()," .. userid .. ",'" .. partfilename .. "')";
   con:execute(query1); 
...
d = tonumber(use());
freeswitch.consoleLog("info", script_name .. " : The user is = " .. userid .. " and have chosen subject " .. subjects[d] .. "\n");
if (d > 0) then
   subject_handler(subjectid[d]);
end
end
---------------------------
--Subject Handler
...
repeat
   ...
   read(aosd .. "/Digits/1.wav", 0);
   read(aosd .. "/voiceprompt/Another_Q_A.wav",0);
   read(aosd .. "/Digits/2.wav", 0);
   sleep(500);
   d = tonumber(use());
...
if (d==1) then
   ...
   elseif (d==2) then
        ---Select Question
      query = "select QuestionID, file from I_question where Askedby = " .. userid .. " AND subject_id = " .. subj_id;
      select_Q(query);
   end
   until (d ~=nil);
elseif (d==2) then
...
freeswitch.consoleLog("info", script_name .. " : Database Updation taking place " .. "\n");
   ...
   query1 = "insert into I_question (Askedby , subject_id ,posting_date, file) values (" .. askedby .. "," .. subj_id .. ",curdate(),'" .. partfilename .. "')";
   con:execute(query1); 
...
query="select file from I_answer where questionid = " .. Q_id;
...
--read(aosd .. "/Rang Rang.mp3", 500);
   ...
   userid = session:read(1, 10, aosd .. "/System/userid.wav", 2000, "#");
